...
export class DipStepperComponent {
  ...
  @ViewChild('volStepper', { static: true }) volStepper!: MatStepper;
  ...
  @ViewChild('volContainer', { static: true, read: ElementRef }) volContainer!: ElementRef<HTMLElement>;

  @ViewChild(MatTabGroup) tabGroup!: MatTabGroup;

  private currentBlock: 'decl' | 'int' | 'vol' = 'decl';
  activeDeclName$: Observable<any> = this.state.activeName$.pipe(map(d => d));

  declaranteActivoName: string = '';
  ...
  private volIndexSubject = new BehaviorSubject<number>(-1);
  volIndex$ = this.volIndexSubject.asObservable();
  ...
  volProgress$ = this.state.volProgress$;
  ...
  volSteps$ = this.state.voluntariasSteps$.pipe(distinctUntilChanged());
  ...
  ngOnInit(): void {
    ...
    this.state.interesesSteps$
      .pipe(takeUntil(this.destroy$))
      .subscribe(s => {
        this._intSteps = s.filter(step => typeof step.step === 'number' && step.step < 14);
        // Solo actualizar el índice si estamos cambiando de pestaña o seleccionando un nuevo declarante
        if (this.selectedTabIndexSubject.value === 1 && this._intSteps.length > 0 && this.intIndexSubject.value === -1) {
    ...
    // Suscripción a los pasos de declaraciones voluntarias
    this.state.voluntariasSteps$
      .pipe(takeUntil(this.destroy$))
      .subscribe(s => {
        this._volSteps = s.filter(step => typeof step.step === 'number' && step.step >= 14);
        // Solo actualizar el índice si estamos cambiando de pestaña
        if (this.selectedTabIndexSubject.value === 2 && this._volSteps.length > 0 && this.volIndexSubject.value === -1) {
          this.volIndexSubject.next(0);
          this.handleVolChange({ selectedIndex: 0 } as StepperSelectionEvent);
        }
        // Notificar al ChangeDetector
        this.cd.markForCheck();
      });
  ...
  get volIndex(): number {
    return this.volIndexSubject.value;
  }

  set volIndex(value: number) {
    this.volIndexSubject.next(value);
  }
  ...
  private syncVisualStepper(key: string): void {
    ...
    if (this.currentBlock === 'decl') {
      ...
      const idxInt = this.intSteps.findIndex(s => s.key === key);
      const idxVol = this.volSteps.findIndex(s => s.key === key);

      if (idxInt >= 0 && this.currentBlock === 'int') {
        console.log('Índice encontrado en intereses:', idxInt);
        if (this.intStepper.selectedIndex !== idxInt) {
          const step = this.intSteps[idxInt];
          console.log('Paso encontrado:', step);
          if (step && step.enabled) {
            console.log('Actualizando stepper intereses a índice:', idxInt);
            this.intStepper.selectedIndex = idxInt;
            this.intIndex = idxInt;
      ...
      } else if (idxVol >= 0 && this.currentBlock === 'vol') {
        console.log('Índice encontrado en voluntarias:', idxVol);
        if (this.volStepper.selectedIndex !== idxVol) {
          const step = this.volSteps[idxVol];
          console.log('Paso encontrado:', step);
          if (step && step.enabled) {
            console.log('Actualizando stepper voluntarias a índice:', idxVol);
            this.volStepper.selectedIndex = idxVol;
            this.volIndex = idxVol;
            const componentName = step.component;
            if (componentName && COMPONENT_MAP[componentName]) {
              console.log('Actualizando componente activo:', componentName);
              this.activeComponent = COMPONENT_MAP[componentName];
  ...
  private advanceVisualStepper(): void {
    ...
    this.state.currentStepKey$.pipe(take(1)).subscribe(currentStep => {
      ...
      if (this.currentBlock === 'decl' && currentStep === 'paso5') {
        ...
        // Notificar cambios
        this.cd.markForCheck();
      } else if (this.currentBlock === 'int' && currentStep === 'paso14') {
        // Avanzando del paso 13 al paso 14
        console.log('Avanzando del paso-13 al paso-14, actualizando tab');
        this.selectedTabIndexSubject.next(2); // Cambiar a Declaraciones Voluntarias
        this.currentBlock = 'vol';
        this.cd.markForCheck();
      }
    });
    ...
    } else if (this.currentBlock === 'int') {
      this.center(this.intContainer);
    } else {
      this.center(this.volContainer);
    }
  ...
  private resetToBlock(b: any): void {
    ...
    if (b === 'decl') {
      if (this._declSteps && this._declSteps.length > 0) {
        ...
        if (this._volSteps && this._volSteps.length > 0) {
          this.volIndexSubject.next(0);
          this.volStepper.selectedIndex = 0;
          this.handleVolChange({ selectedIndex: 0 } as StepperSelectionEvent);
        } else {
          this.volIndexSubject.next(-1);
          this.volStepper.selectedIndex = -1;
        }
      }
  ...
  onTabChange(event: any): void {
    ...
    if (newIndex === 0) {
      ...
      of(null).subscribe(() => {
        const currentVolIndex = this.volIndexSubject.value;
        if (currentVolIndex === -1) {
          this.volIndexSubject.next(0);
          this.volStepper.selectedIndex = 0;
        }
        this.loadVolStep(currentVolIndex === -1 ? 0 : currentVolIndex);
      });
    }
  ...
  selectDeclarante(id: string): void {
    ...
    // Solo actualizamos el stepper de intereses o voluntarias dependiendo de la pestaña activa
    if (this.selectedTabIndex === 1) {
      this.state.setActiveBlock('int');
      this.intIndex = -1; // Resetear el índice para forzar la carga del primer paso
    } else if (this.selectedTabIndex === 2) {
      this.state.setActiveBlock('vol');
      this.volIndex = -1; // Resetear el índice para forzar la carga del primer paso
    }
    ...
    setTimeout(() => {
      if (this.selectedTabIndex === 1) {
        ...
        if (this.volSteps && this.volSteps.length > 0) {
          const firstEnabledStep = this.volSteps.findIndex(step => step.enabled);
          if (firstEnabledStep >= 0) {
            this.volStepper.selectedIndex = firstEnabledStep;
            this.volIndex = firstEnabledStep;
            const step = this.volSteps[firstEnabledStep];
            const componentName = step.component;
            if (componentName && COMPONENT_MAP[componentName]) {
              this.activeComponent = COMPONENT_MAP[componentName];
            }
  ...
  handleDeclChange(ev: StepperSelectionEvent): void {
    ...
    if (!this._declSteps || this._declSteps.length === 0) {
      ...
      this.intStepper.selectedIndex = -1;
      this.intIndexSubject.next(-1);
      // Limpiar el stepper de voluntarias
      this.volStepper.selectedIndex = -1;
      this.volIndexSubject.next(-1);
  ...
  /* VOLUNTARIAS (Nuevo) */
  handleVolChange(ev: StepperSelectionEvent): void {
    if (!this._volSteps || this._volSteps.length === 0) {
      this.volIndex = -1;
      return;
    }

    const targetStep = this._volSteps[ev.selectedIndex];

    if (targetStep && targetStep.enabled) {
      this.volIndex = ev.selectedIndex;
      ...
      this.volStepper.selectedIndex = this.volIndex;
    }
    this.cd.markForCheck();
  }

  /* ───────── Helpers de carga ───────── */
  loadStep(type: 'decl' | 'int' | 'vol', idx: number): void {
    console.log('loadStep', type, idx);
    const step = type === 'decl'
      ? (this._declSteps[idx] || null)
      : type === 'int'
        ? (this._intSteps[idx] || null)
        : (this._volSteps[idx] || null);
  ...
  private loadVolStep(i: number): void { this.loadStep('vol', i); }
  ...
  private attachHeaderClicks(): void {
    ...
    /* VOLUNTARIAS (Nuevo) */
    this.volContainer.nativeElement
      .querySelectorAll<HTMLElement>('.mat-step-header')
      .forEach((hdr, idx) => {
        hdr.style.cursor = 'pointer';
        hdr.addEventListener('click', () => {
          const step = this._volSteps[idx];
          if (step && step.enabled) {
            if (this.volIndex === idx) {
              this.loadVolStep(idx);
            } else {
              this.volStepper.selectedIndex = idx;
              this.handleVolChange({ selectedIndex: idx } as StepperSelectionEvent);
            }
          }
        });
      });
  ...
  getVolStatus(i: number): Step['status'] { return this._volSteps[i]?.status ?? 'pending'; }
