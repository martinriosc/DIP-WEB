...
import { JefeServicioService, ProcesoMasivo, Exportacion } from '../../services/jefe-servicio.service';
...
@Component({
  selector: 'app-jefe-servicio-list',
  standalone:false,
  templateUrl: './jefe-servicio-list.component.html',
  styleUrls: ['./jefe-servicio-list.component.scss']
})
export class JefeServicioListComponent implements OnInit {
  /* ──────────────────────────── Tabs ──────────────────────────── */
  pendientesCount = 0;
  firmaEnvioCount = 0;
  enviadasCount   = 0;
...
isLoading   = false;
  ...
  dataSourcePendientes  = new MatTableDataSource<DeclaracionJefeServicio>([]);
  dataSourceEnviadas    = new MatTableDataSource<DeclaracionJefeServicio>([]);
  dataSourceArchivadas  = new MatTableDataSource<DeclaracionJefeServicio>([]);
  ...
  selectionPendientes = new SelectionModel<DeclaracionJefeServicio>(true, []);
  selectionEnviadas   = new SelectionModel<DeclaracionJefeServicio>(true, []);
  selectionArchivadas = new SelectionModel<DeclaracionJefeServicio>(true, []);
  ...
  constructor(
    ...
    private _jefeServicio: JefeServicioService,
  ...
  private loadBandeja(page = 0, limit = 100): void {
    ...
    this._declaracion
      .listar<DeclaracionJefeServicio>(filtro, 'id', page, limit)
      .pipe(
    ...
    getDataSource(tab: TabId): MatTableDataSource<DeclaracionJefeServicio> {
  ...
  private bindToDatasource(arr: DeclaracionJefeServicio[]): void {
  ...
  getSelection(tab: TabId): SelectionModel<DeclaracionJefeServicio> {
  ...
  toggle(tab: TabId, row: DeclaracionJefeServicio): void {
    this.getSelection(tab).toggle(row);
  }

  isSelected(tab: TabId, row: DeclaracionJefeServicio): boolean {
  ...
  onFirmar(): void {
    ...
    if (selectedItems.length === 1) {
      ...
      this.mostrarModalConfirmacion(
        ...
        () => {
          this._jefeServicio.firmarDeclaracion(element.idDeclaracion, this.observacionModal.value || '').subscribe({
            next: (response) => {
  ...
  onDerivar(): void {
    ...
    if (selectedItems.length === 1) {
      ...
      this.mostrarModalConfirmacion(
        ...
        () => {
          this._jefeServicio.enviarVisador(element.idDeclaracion, idVisadorDefault, this.observacionModal.value || '').subscribe({
            next: (response) => {
  ...
  cargarProcesosMasivos(): void {
    ...
    if (this.currentTab === 'pendientes') {
      this._jefeServicio.listarProcesosMasivos().subscribe({
        next: (response) => {
      ...
      this._jefeServicio.listarProcesosFirmarArchivarMasivos().subscribe({
        next: (response) => {
          this.procesosMasivos = response.data || [];
          this.procesosMasivosLoading = false;
        },
  ...
  cargarExportaciones(): void {
    ...
    this._jefeServicio.listarExportaciones().subscribe({
      next: (response) => {
  ...
  iniciarExportacion(): void {
    ...
    this._jefeServicio.iniciarExportacion(filtro).subscribe({
      next: (response) => {
   ...
   */
  descargarDeclaracion(element: DeclaracionJefeServicio): void {
    this._jefeServicio.descargarDeclaracion(element.idDeclaracion).subscribe({
      next: (blob) => {
  ...
  archivarSeleccionadas(): void {
    ...
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.archivarSeleccionadas(selectedIds, this.observacionModal.value || '').subscribe({
          next: (response) => {
  ...
  devolverSeleccionadas(): void {
    ...
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.devolverSeleccionadas(selectedIds, this.observacionModal.value || '').subscribe({
          next: (response) => {
  ...
  firmarEnviarSeleccionadas(): void {
    ...
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.firmarEnviarSeleccionadas(selectedIds, this.observacionModal.value || '').subscribe({
          next: (response) => {
  ...
  devolverTodasDeclaraciones(): void {
    ...
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.devolverTodoDeclaracion(filtro, this.observacionModal.value || '').subscribe({
          next: (response) => {
  ...
  firmarArchivarTodasDeclaraciones(): void {
    ...
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.firmarArchivarMasivo(filtro, this.observacionModal.value || '').subscribe({
          next: (response) => {
   ...
   */
  enviarOrganismoFiscalizador(element: DeclaracionJefeServicio): void {
    this.mostrarModalConfirmacion(
      ...
      () => {
        this._jefeServicio.enviarCgrDeclaracion(element.idDeclaracion, this.observacionModal.value || '').subscribe({
          next: (response) => {
  ...
  descargarExportacion(exportacion: Exportacion): void {
    ...
    const url = `${this._jefeServicio['apiUrl']}/pr/service/exportaciones/descargar/${exportacion.id}`;
    const link = document.createElement('a');
    link.href = url;
